...
static int	all_coders_done(t_monitor *monitor, t_simulation *sim)
{
	...
	while (i < monitor->info->number_of_coders)
	{
		...
		if (monitor->coders[i].compiles_done < monitor->info->number_of_compiles_required)
		{
...
static int	check_burnout(t_monitor *monitor, t_simulation *sim)
{
	int		i;
	long	now;
	t_coder	*cdr = monitor->coders;
	t_shared_info *info = monitor->info;
	...
	{
		pthread_mutex_lock(&sim->mutex);
		if (cdr[i].compiles_done < info->number_of_compiles_required
			&& now - cdr[i].last_time_compilation >= info->time_to_burnout)
		{
			pthread_mutex_unlock(&sim->mutex);
			pthread_mutex_lock(&info->print_mutex);
			printf("%ld %d burned out\n", now - info->start_time, cdr[i].id);
